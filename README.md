# Zenvy — vidéo kinetic typography (15 s, 9:16)
...
**Livrable :** `out/zenvy_kinetic_15s.mp4` — 1080×1920, 60 fps, 15,000 s,
...
| Durée | 15 s (montage entièrement piloté par `timing.py`) |
...
|---|---|---|
| 0 – 2,9 s | *On a tous ce moment où on veut sortir…* | les mots arrivent en glissant des côtés en alternance, toutes les 58 ms, avec un vrai filé de mouvement ; bloc en place en ~0,5 s, push-in pendant la tenue, sortie en vrille vers la gauche |
| 2,9 – 5,3 s | *…mais personne n'est dispo.* | cassure : le bloc tombe en 0,16 s avec flou de mouvement, s'écrase à l'atterrissage (squash amorti, pieds ancrés), secousse caméra, gris froid, sortie vers le haut |
| 5,3 – 7,6 s | *Ou on est dispo…* | reprise, cadence 54 ms, sortie en vrille vers la droite |
| 7,6 – 10 s | *…mais on sait pas où aller.* | même mécanique de chute que la première |
| 10 – 12,2 s | *Et si on savait tout, tout de suite ?* | cadence 42 ms, push-in plus marqué, accent orange sur « TOUT DE SUITE ? », sortie en zoom vers la caméra |
| 12,2 – 14 s | **logo + Zenvy** | flash, pop d'échelle avec dépassement, interlettrage qui se resserre, halo orange pulsé, whoosh + pop sonore |
| 14 – 15 s | *Lancement le 20 août à Bordeaux* | la marque recule en haut, filet accent + carton final fixe |

Le rythme accélère en continu jusqu'à la révélation (58 → 54 → 42 ms entre les
mots, pulsations audio qui se resserrent de 1,35 s à 0,2 s), puis se stabilise
sur le carton final.
...
```bash
python3 zenvy_kinetic/render_video.py --stills 0.9,3.2,10.9,12.5,14.5
```
